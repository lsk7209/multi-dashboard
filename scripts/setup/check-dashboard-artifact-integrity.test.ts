...
describe("check-dashboard-artifact-integrity", () => {
  ...
  it("accepts a current ready-to-act state after external recovery", () => {
    const result = evaluateDashboardArtifactIntegrity(
      makeInput({
        ...
        postRecovery: makePostRecovery({
          ...
          dashboardVerification: {
            path: join("data", "dashboard-verification-2026-07-06.json"),
            statsSnapshot: SNAPSHOT,
            verdict: "local_verified",
            expectedBlocked: 0,
            fail: 0,
  ...
  it("fails when the UI smoke artifact is stale against the current snapshot", () => {
    ...
    expect(
      result.checks.find((check) => check.id === "current_snapshot_aligned"),
    ).toMatchObject({
      pass: false,
    });
  });
  ...
  it("fails an external blocker verdict without concrete GSC blocker fields", () => {
          ...
          externalBlockerEvidence: [
            {
              ...
              artifactPath: join(
                "data",
                "gsc-permission-audit-2026-07-06.json",
              ),
              workOrderPath: join(
                "docs",
                "work-orders",
                "gsc-permission-audit-2026-07-06.md",
              ),
    ...
    expect(result.ready).toBe(false);
    expect(
      result.checks.find((check) => check.id === "verdict_state_consistent"),
    ).toMatchObject({
      pass: false,
    });
  });
  ...
  it("fails when post-recovery snapshot disagrees with dashboard verification", () => {
    ...
    expect(
      result.checks.find(
        (check) => check.id === "post_recovery_matches_verification",
      ),
    ).toMatchObject({
      pass: false,
    });
  ...
  it("fails when Markdown work-order summaries drift from JSON", () => {
    ...
    expect(result.ready).toBe(false);
    expect(
      result.checks.find((check) => check.id === "markdown_mirrors_json"),
    ).toMatchObject({
      pass: false,
    });
  });
  ...
  it("fails pass command results with dirty stderr or non-zero exits", () => {
    ...
    expect(
      result.checks.find(
        (check) => check.id === "commands_and_tails_consistent",
      ),
    ).toMatchObject({
      pass: false,
    });
  ...
  it("fails acceptance rows without requirement and evidence text", () => {
    ...
    expect(result.ready).toBe(false);
    expect(
      result.checks.find((check) => check.id === "post_recovery_rows_exact"),
    ).toMatchObject({
      pass: false,
    });
  });
  ...
  it("fails external blocker audits when collectorSnapshot only contains the snapshot as a substring", () => {
    const result = evaluateDashboardArtifactIntegrity(
      makeInput({
        auditArtifacts: {
          [join("data", "gsc-permission-audit-2026-07-06.json")]: {
    ...
    expect(result.ready).toBe(false);
    expect(
      result.checks.find((check) => check.id === "blocker_evidence_concrete"),
    ).toMatchObject({
      pass: false,
    });
  });
  ...
  it("fails external blocker audits when handoffStatus is missing or inconsistent", () => {
    const missingStatus = evaluateDashboardArtifactIntegrity(
      makeInput({
        auditArtifacts: {
          [join("data", "gsc-permission-audit-2026-07-06.json")]: {
            collectorSnapshot: `data/site-stats.json generatedAt=${SNAPSHOT}`,
    ...
    expect(
      missingStatus.checks.find(
        (check) => check.id === "blocker_evidence_concrete",
      ),
    ).toMatchObject({
      pass: false,
    });

    const inconsistentStatus = evaluateDashboardArtifactIntegrity(
      makeInput({
        auditArtifacts: {
          [join("data", "gsc-permission-audit-2026-07-06.json")]: {
            collectorSnapshot: `data/site-stats.json generatedAt=${SNAPSHOT}`,
    ...
    expect(
      inconsistentStatus.checks.find(
        (check) => check.id === "blocker_evidence_concrete",
      ),
    ).toMatchObject({
      pass: false,
  ...
  it("fails external blocker audits when the GSC work-order Markdown drifts from JSON", () => {
    const result = evaluateDashboardArtifactIntegrity(
      makeInput({
        auditMarkdowns: {
          [join("docs", "work-orders", "gsc-permission-audit-2026-07-06.md")]:
            makeGscAuditMarkdown({
              handoffStatus: "resolved",
            }),
        },
      }),
  ...
  it("fails when the recorded post-write artifact integrity result failed", () => {
    ...
    expect(result.ready).toBe(false);
    expect(
      result.checks.find((check) => check.id === "post_write_integrity_clean"),
    ).toMatchObject({
      pass: false,
    });
  });
...
function makeGscAuditMarkdown(
  ...
  const snapshot =
    overrides.snapshot ?? `data/site-stats.json generatedAt=${SNAPSHOT}`;
  ...
  const configuredGscSiteUrl =
    overrides.configuredGscSiteUrl ?? "https://yesa.kr/";
  ...
  const requiredAction =
    overrides.requiredAction ?? "Verify the Search Console property.";
...
function makeVerification(overrides: Record<string, unknown> = {}) {
  return {
    ...
    externalBlockerEvidence: [
      {
        source: "gsc_permission_audit",
        artifactPath: join("data", "gsc-permission-audit-2026-07-06.json"),
        workOrderPath: join(
          "docs",
          "work-orders",
          "gsc-permission-audit-2026-07-06.md",
    ...
    renderedUiSmokeEvidence: {
      path: join("data", "dashboard-ui-smoke-2026-07-06.json"),
...
function makeVerificationMarkdown(
  overrides: {
    verdict?: string;
  ...
  const actionabilityStatus =
    overrides.actionabilityStatus ??
    "blocked_for_action_until_post_recovery_verify";
  const requiredCommand =
    overrides.requiredCommand ?? "pnpm dashboard:post-recovery";
  ...
  const requiredAction =
    overrides.requiredAction ?? "Verify the Search Console property.";
  const workOrderPath =
    overrides.workOrderPath ??
    join("docs", "work-orders", "gsc-permission-audit-2026-07-06.md");
  const acceptanceRows = makePostRecoveryAcceptance(
    verdict === "local_verified" ? "satisfied" : "mixed",
  );
...
function makePostRecoveryMarkdown(
  overrides: {
    readiness?: string;
    verdict?: string;
    actionabilityStatus?: string;
    ...
    results: [
      {
        ...
        status: "pass",
        exitCode: 0,
        stdoutTail: `Wrote ${join("data", "gsc-permission-audit-2026-07-06.json")}. snapshot=2026-07-05T19:00:00.000Z\npre_refresh_non_authoritative=true finalDashboardVerificationSnapshot=${SNAPSHOT}`,
      },
      {
        id: "dashboard-verify",
        status: "pass",
...
function requirementForAcceptanceId(id: string): string {
  const requirements: Record<string, string> = {
    ...
    rendered_ui_smoke_current:
      "Rendered browser UI smoke exists and matches the current stats snapshot.",
    dashboard_surface_current:
      "Dashboard runtime surface evidence matches the current stats snapshot.",
    recommendations_safe_to_act:
      "Dashboard actions and insights are execution-ready, not read-only.",
    mutation_boundary_clean:
      "Verification remains non-mutating for production, CMS, GSC, AdSense, and title/body content.",
  };
...
function evidenceForAcceptanceId(id: string, status: string): string {
  const evidence: Record<string, string> = {
    external_gsc_access_restored:
      status === "satisfied"
        ? "No external GSC blockers remain."
        : "Pending blockers: yesa.kr.",
    dashboard_verify_local_verified:
      status === "satisfied"
        ? "Current verdict is local_verified."
        : "Current verdict is local_verified_external_blocker.",
    ...
    recommendations_safe_to_act:
      status === "satisfied"
        ? "Current actionability is safe_to_act."
        : "Current actionability is blocked_for_action_until_post_recovery_verify.",
    mutation_boundary_clean:
